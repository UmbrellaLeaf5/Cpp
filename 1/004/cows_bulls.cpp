...
vector<char> get_guess ()
{
    ...
    if (numbers.size() > 4)
    {
        throw runtime_error("Error entering number. The number has too much digits. Please follow "
    ...
    try
    {
        ...
        for (bool run_game = true; run_game;)
        {
            ...
            auto [bulls, cows] = calc_bulls_cows(
                guess, solution);  // Количество "быков" и "коров" = функции, которая их считает
            ...
            {
                cout << "You are close to victory. There are " << bulls << " bulls and " << cows
                     << " cows. Try again!" << endl
                     << endl;
            }
        }
