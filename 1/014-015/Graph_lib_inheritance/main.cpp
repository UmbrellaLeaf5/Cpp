...
class Regular_hexagon : public Shape {
 public:
  ...
  Regular_hexagon(Point c, int r) {
    this->add({c.x - r, c.y});
    this->add({c.x - r + r * cos(Graph_lib::pi / 3.0),
               c.y + r * sin(Graph_lib::pi / 3.0)});
    this->add({c.x + r * cos(Graph_lib::pi / 3.0),
               c.y + r * sin(Graph_lib::pi / 3.0)});
    this->add({c.x + r, c.y});
    this->add({c.x + r * cos(Graph_lib::pi / 3.0),
               c.y - r * sin(Graph_lib::pi / 3.0)});
...
void Fibonacci_spiral() {
  ...
  for (int i = 1; i <= count; i++) {
    ...
    arcs.push_back(new MyArc{
        {curr_x, curr_y}, curr_size, curr_size, curr_angle, curr_angle + 90});
...
int main() try {
  ...
  // vector <MyArc> arcs;
  // arcs.push_back(arc); - так как в Shape явно запрещено копирование, этот метод не работает для
  // vector Fibonacci_spiral();
