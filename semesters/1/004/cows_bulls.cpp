...
vector<char> get_guess() {
  ...
  for (char temp; cin >> temp;)  /// Вот тут ошибка!!!
  {
    if (cin.fail()) {
      throw runtime_error(
          "Error entering number. Please follow the rules of the game.");
    }
    numbers.push_back(temp);
  }
  if (numbers.size() > 4) {
    throw runtime_error(
        "Error entering number. The number has too much digits. Please follow "
  ...
  /*for(size_t i = 0; i < nsize; i++) {
        for(size_t j = i; j < nsize; j++){
            ...
            {
                throw runtime_error("Error entering number. The digits in the number are repeated.
...
int main() {
  try {
    vector<char> solution =
        gen_solution();  // Загаданное число = сгенерированному
    // Стартовое сообщение
    cout << "<<Bulls and Cows>>\n"
    ...
    for (bool run_game = true; run_game;) {
      vector<char> guess =
          get_guess();  // Текущая попытка = получить ввод числа от пользователя
      auto [bulls, cows] = calc_bulls_cows(
          guess,
          solution);  // Количество "быков" и "коров" = функции, которая их считает
