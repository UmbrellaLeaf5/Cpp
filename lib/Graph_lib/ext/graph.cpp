...
Arch::Arch(Point p, int ww, int hh, int alpha, int beta)
...
Box::Box(Point xy, int ww, int hh, int radius) : Rectangle{xy, ww, hh}, r{radius}
{
    if (r < 0)
...
Point Regular_hexagon::center() const { return {(point(0).x + point(3).x) / 2, point(0).y}; }
...
{
    std::array<Color, 3> colors{Color::dark_cyan, Color::dark_magenta, Color::dark_yellow};
